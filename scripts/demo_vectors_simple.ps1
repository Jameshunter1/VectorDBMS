...
function Add-Vector {
 ...
 $builder = [System.UriBuilder]::new("$baseUrl/api/vector/put")
 # Escape & with backtick for PowerShell
 $builder.Query = "key=$key`&vector=$vectorStr"

 try {
  Invoke-RestMethod -Uri $builder.Uri -Method Post -ContentType "application/json" | Out-Null
  return $true
 }
 catch {
...
# Step 1: Populate User Profile Embeddings
Write-Host "=== Step 1: Populate User Profile Embeddings ===" -ForegroundColor Yellow
Write-Host "Inserting 5 user vectors - 128 dimensions..." -ForegroundColor Yellow
Write-Host ""
...
# Step 2: Populate Document Embeddings
Write-Host "=== Step 2: Populate Document Embeddings ===" -ForegroundColor Yellow
Write-Host "Inserting 8 document vectors - 128 dimensions..." -ForegroundColor Yellow
Write-Host ""
...
# Step 3: Populate Image Embeddings
Write-Host "=== Step 3: Populate Image Embeddings ===" -ForegroundColor Yellow
Write-Host "Inserting 7 image vectors - 128 dimensions..." -ForegroundColor Yellow
Write-Host ""
...
# Step 4: Vector Database Statistics
...
Write-Host "`nSearching for similar vectors to first user..." -ForegroundColor Yellow

# Generate a random query vector
$queryVec = New-RandomVector -dim 128
$queryStr = $queryVec -join ","

$searchBuilder = [System.UriBuilder]::new("$baseUrl/api/vector/search")
$searchBuilder.Query = "vector=$queryStr`&k=5"

 $results = Invoke-RestMethod -Uri $searchBuilder.Uri -Method Get
