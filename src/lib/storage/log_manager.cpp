...
std::vector<std::byte> UpdateLogRecord::Serialize() const {
  ...
  std::size_t total_size = base_size + sizeof(PageId) + sizeof(std::size_t) * 2 
...
LogManager::LogManager(const std::string& log_file)
  ...
  Log(LogLevel::kInfo, "LogManager initialized, log file: " + log_file +
      ", offset: " + std::to_string(log_offset_));
}
...
LSN LogManager::AppendUpdateRecord(TxnId txn_id, LSN prev_lsn, PageId page_id,
                                   std::size_t offset, std::size_t length,
                                   const std::byte* old_data, const std::byte* new_data) {
  auto record = std::make_unique<UpdateLogRecord>(
      txn_id, prev_lsn, page_id, offset, length, old_data, new_data);
  return AppendLogRecord(std::move(record));
}

LSN LogManager::AppendCLRRecord(TxnId txn_id, LSN prev_lsn, PageId page_id,
                                std::size_t offset, std::size_t length,
                                const std::byte* undo_data, LSN undo_next_lsn) {
  auto record = std::make_unique<CLRLogRecord>(
      txn_id, prev_lsn, page_id, offset, length, undo_data, undo_next_lsn);
  return AppendLogRecord(std::move(record));
}
...
Status LogManager::ScanForward(LSN start_lsn,
