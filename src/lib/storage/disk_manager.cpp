...
Status DiskManager::Open() {
  ...
  Log(LogLevel::kInfo, "DiskManager opened: " + db_file_.string() + 
...
Status DiskManager::ReadPage(PageId page_id, Page* page) {
  ...
  if (page->GetPageId() != page_id && page->GetPageId() != kInvalidPageId) {
    return Status::Corruption("Page ID mismatch: expected " + std::to_string(page_id) +
...
Status DiskManager::WritePage(PageId page_id, const Page* page) {
  ...
  if (!page->VerifyChecksum()) {
    return Status::InvalidArgument("Page checksum not valid - call UpdateChecksum() before WritePage");
  }
...
PageId DiskManager::AllocatePage() {
  ...
  const std::size_t bytes_written = std::fwrite(empty_page.GetRawPage(), 1, kPageSize, file_handle_);
