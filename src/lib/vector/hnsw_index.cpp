...
HNSWIndex::HNSWIndex(Params params) 
...
std::vector<HNSWIndex::SearchResult> HNSWIndex::Search(const Vector& query, std::size_t k) const {
  ...
  for (std::size_t i = 0; i < std::min(k, candidates.size()); ++i) {
    ...
    if (!nodes_[node_id].deleted) {
      results.push_back({
        nodes_[node_id].key,
        Distance(query, nodes_[node_id].vector)
      });
    }
  }
...
std::vector<int> HNSWIndex::SearchLayer(const Vector& query, int entry_point, std::size_t ef, int layer) const {
  ...
  std::priority_queue<std::pair<float, int>, std::vector<std::pair<float, int>>, decltype(cmp_results)> results(cmp_results);
...
void HNSWIndex::SelectNeighbors(int node_id, const std::vector<int>& candidates, std::size_t M, int layer) {
...
void HNSWIndex::PruneConnections(int node_id, int layer) {
  ...
  if (neighbors.size() <= params_.M) return;
