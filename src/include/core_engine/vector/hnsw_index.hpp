...
class HNSWIndex {
 public:
  ...
  struct Params {
    ...
    std::size_t ef_construction = 200;           // Size of dynamic candidate list during construction (higher = better quality, slower inserts)
    std::size_t ef_search = 50;                  // Size of dynamic candidate list during search (higher = better recall, slower queries)
  ...
  DistanceMetric metric() const { return params_.metric; }
 ...
 private:
  ...
  std::vector<int> SearchLayer(const Vector& query, int entry_point, std::size_t ef, int layer) const;
