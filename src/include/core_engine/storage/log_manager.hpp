...
struct UpdateLogRecord : public LogRecord {
  ...
  UpdateLogRecord(TxnId txn_id, LSN prev_lsn, PageId page_id,
                  std::size_t offset, std::size_t length,
                  const std::byte* old_val, const std::byte* new_val) {
    this->txn_id = txn_id;
    this->prev_lsn = prev_lsn;
...
struct CLRLogRecord : public LogRecord {
  ...
  CLRLogRecord(TxnId txn_id, LSN prev_lsn, PageId page_id,
               std::size_t offset, std::size_t length,
...
class LogManager {
 public:
  ...
  // Returns: LSN of the log record
  LSN AppendUpdateRecord(TxnId txn_id, LSN prev_lsn, PageId page_id,
                         std::size_t offset, std::size_t length,
  ...
  LSN AppendCLRRecord(TxnId txn_id, LSN prev_lsn, PageId page_id,
                      std::size_t offset, std::size_t length,
  ...
  LSN GetNextLSN() const { return next_lsn_; }
  ...
  Status ScanForward(LSN start_lsn, 
  ...
  // Returns: Status of scan operation
  Status ScanBackward(LSN start_lsn,
                     std::function<void(const LogRecord&)> callback);
