...
TEST_CASE("Storage Layer: DiskManager", "[storage][disk]") {
  ...
  const auto db_dir =
      std::filesystem::temp_directory_path() / ("test_disk_manager_" + std::to_string(suffix));
...
TEST_CASE("Storage Layer: BufferPoolManager", "[storage][buffer]") {
  const auto suffix = static_cast<std::uint64_t>(
      std::chrono::high_resolution_clock::now().time_since_epoch().count());
  const auto db_dir =
      std::filesystem::temp_directory_path() / ("test_buffer_pool_" + std::to_string(suffix));
...
TEST_CASE("Storage Layer: LogManager", "[storage][wal]") {
  const auto suffix = static_cast<std::uint64_t>(
      std::chrono::high_resolution_clock::now().time_since_epoch().count());
  const auto wal_file =
      std::filesystem::temp_directory_path() / ("test_wal_" + std::to_string(suffix) + ".log");
  ...
  SECTION("Basic log operations") {
    ...
    auto update_lsn = lm.AppendUpdateRecord(txn_id, begin_lsn, 1, 0, 3, old_data, new_data);
